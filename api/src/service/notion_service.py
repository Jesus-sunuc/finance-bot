...
class NotionService:
    ...
    def create_expense(
        ...
        try:
            ...
            properties = self._build_expense_properties(
                actual_properties, amount, category, merchant, date, description
            )
        ...
        for prop_name, prop_data in actual_properties.items():
            if prop_data.get("type") == "title":
                properties[prop_name] = {
                    "title": [{"text": {"content": f"{merchant} - ${amount}"}}]
                }
                break
